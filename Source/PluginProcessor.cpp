...
void ReverbAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    const int maxDelayInSamples = (int)std::ceil(5.0 * sampleRate);
    combL.prepare(sampleRate, maxDelayInSamples);
    combR.prepare(sampleRate, maxDelayInSamples);

    int delayInSamples = (int)(0.15 * sampleRate);
    combL.setDelaySamples(delayInSamples);
    combR.setDelaySamples(delayInSamples + 7);
...
void ReverbAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    params.update();
    const int maxDelayInsamples = combL.getMaxDelayInSamples();
    const int delayInSamples = juce::jlimit(1, maxDelayInsamples, int(std::round(params.delayTime * fs)));
    combL.setDelaySamples(delayInSamples);
    combR.setDelaySamples(delayInSamples + 7);
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
        ...
        float wetL = combL.processSample(dryL);
        float wetR = combR.processSample(dryR);

        constexpr float wetMix = 1.0f;

        channelDataL[sample] = (1.0f - wetMix) * dryL + wetMix * wetL;
        channelDataR[sample] = (1.0f - wetMix) * dryR + wetMix * wetR;
    }
}
